...
void loadData() {
    ...
    if (fb) {
        while (fscanf(fb, "%d %s %d %s %d", &id, name, &age, phone, &seat) != EOF) {
            ...
            node->id = id; strcpy(node->name, name);
            node->age = age; strcpy(node->phone, phone);
            node->seatNo = seat;
...
void enqueue(int id, char name[], int age, char phone[]) {
    ...
    if (!waitRear) waitFront = waitRear = node;
    else { waitRear->next = node; waitRear = node; }
}
    ...
    for (i = bookedHead; i && i->next; i = i->next)
        for (j = i->next; j; j = j->next)
            if (i->seatNo > j->seatNo) {
                BookNode t = *i;
                i->seatNo=j->seatNo; j->seatNo=t.seatNo;
                i->id=j->id; j->id=t.id;
                i->age=j->age; j->age=t.age;
                strcpy(i->name,j->name); strcpy(j->name,t.name);
                strcpy(i->phone,j->phone); strcpy(j->phone,t.phone);
            }
}
